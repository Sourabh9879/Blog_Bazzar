...
router.post('/create-post', async (req, res) => {
    try {
        const { title, content, category, rating, coverImg, createdAt } = req.body;
...
router.post('/send-email', async (req, res) => {
    ...
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            user: 'jis se bhejna hai uska email',
            pass: 'password',
        },
    });

    const mailOptions = {
        from: 'jis se bhejna hai uska email',
        to: email,
        subject: `Blog: ${title}`,
        html: htmlContent, // The blog content to send
    };
